    ...
    $wrapper.click(function (event) {
        ...
        if (dt) {
            ...
            if ($selected.parents('#categories').length) { // category selected
                ...
                html.push('<h1>' + q + ' <span class="type">' + item.type + '</span> <span class="fav' + (item.fav ? ' on' : '') + '" id="' + item.name + item.id + '"></span></h1>');
                html.push('<p class="meta">Category: ' + item.category + '/' + item.subcategory + (item.added ? ' (added ' + item.added + ')' : '') + '</p>');
